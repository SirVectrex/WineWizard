...
public interface WineRepositoryImpl extends WineRepositoryI {
    ...
    @Query("SELECT w.name as name,  w.id as id, ROUND(AVG(r.ratingTaste), 2) as avgTasteRating, ROUND(AVG(r.ratingDesign), 2) as avgDesignRating, ROUND(AVG(r.ratingPrice), 2) as avgPriceRating " +
            ...
            "GROUP BY w.id, w.name " +
            "ORDER BY AVG(r.ratingTaste) DESC")
    Page<WineProjectionI> findWinesWithAverageRatingsByWinery_Pages(Pageable pageable, @Param("winery_id") long winery_id);
    ...
    @Query(value = "SELECT w.NAME as name,  w.id as id, ROUND(AVG(r.TASTE_RATING), 2) as avgTasteRating, ROUND(AVG(r.DESIGN_RATING), 2) as avgDesignRating, ROUND(AVG(r.PRICE_RATING), 2) as avgPriceRating " +
